...
class TripletLoss(nn.Module):
    ...
    def forward(self, source, s_labels, target=None, t_labels=None, margin=0):
        ...
        ma1 = torch.max(anchor_positive_dist)
        mi1 = torch.min(anchor_positive_dist)
        # shape (batch_size, 1, batch_size)
        anchor_negative_dist = pairwise_dist.unsqueeze(1)
        ma2 = torch.max(anchor_negative_dist)
        mi2 = torch.min(anchor_negative_dist)

        triplet_loss = anchor_positive_dist - anchor_negative_dist + margin
        ma3 = torch.max(anchor_positive_dist - anchor_negative_dist)
        mi3 = torch.min(anchor_positive_dist - anchor_negative_dist)

        # Put to zero the invalid triplets
        # (where label(a) != label(p) or label(n) == label(a) or a == p)
        mask, weight = get_triplet_mask(s_labels, t_labels, self.opt)
        triplet_loss1 = mask * triplet_loss

        # Remove negative losses (i.e. the easy triplets)
        triplet_loss = triplet_loss1.clamp(0)

        ma3 = torch.max(triplet_loss)
        mi3 = torch.min(triplet_loss)
